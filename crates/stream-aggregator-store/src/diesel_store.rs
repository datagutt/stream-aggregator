...
impl DieselStore {
	...
	fn get_conn(&self) -> Result<r2d2::PooledConnection<ConnectionManager<SqliteConnection>>, StoreError> {
		self.pool
			.get()
			.map_err(|e| StoreError::ConnectionError(e.to_string()))
	}
...
impl StreamStore for DieselStore {
	async fn upsert_stream(&self, stream: &StreamInfo) -> Result<(), StoreError> {
		...
		tokio::task::spawn_blocking(move || {
			let mut conn = pool.get().map_err(|e| StoreError::ConnectionError(e.to_string()))?;
	...
	async fn get_streams(&self, query: &StreamQuery) -> Result<StreamPage, StoreError> {
		...
		tokio::task::spawn_blocking(move || {
			...
			let total = count_query
				...
				.map_err(|e| StoreError::QueryError(e.to_string()))? as usize;
	...
	async fn get_tracked_streamers(
		...
		tokio::task::spawn_blocking(move || {
			...
			diesel_query = diesel_query
				.order((tracked_streamers::platform.asc(), tracked_streamers::user_id.asc()));
	...
	async fn update_tracked_streamer(&self, streamer: &TrackedStreamer) -> Result<(), StoreError> {
		...
		tokio::task::spawn_blocking(move || {
			...
			let rows_affected = diesel::update(tracked_streamers::table.find((&platform, &user_id)))
				.set(&update)
				.execute(&mut conn)
				.map_err(|e| StoreError::QueryError(e.to_string()))?;
	...
	async fn update_discovery_rule(&self, rule: &DiscoveryRule) -> Result<(), StoreError> {
		...
		tokio::task::spawn_blocking(move || {
			...
			if rows_affected == 0 {
				return Err(StoreError::NotFound(format!("Discovery rule {} not found", id)));
			}
...
mod tests {
	...
	async fn test_diesel_tracked_streamer_operations() {
		...
		let retrieved = store.get_tracked_streamer("twitch", "user123").await.unwrap();
		assert!(retrieved.is_some());
		...
		let deleted = store.get_tracked_streamer("twitch", "user123").await.unwrap();
		assert!(deleted.is_none());
	}
